 ...
 * @version V1.0
 */
package com.yidu.sevensecondmall.views.widget.tagview;
...
public class TagListView extends FlowLayout implements OnClickListener {
	...
	private void inflateTagView(final Tag t, boolean b) {
		...
		int selfWidth = dip2px(mContext, 320);
		int horizontalSpacing = dip2px(mContext , mContext.getResources().getDimension(R.dimen.tag_horizontal_spacing));
		int paddingBound = dip2px(mContext , mContext.getResources().getDimension(R.dimen.tag_common_margin));
		int itemHeight = dip2px(mContext , mContext.getResources().getDimension(R.dimen.tag_height));
		int itemWidth = (selfWidth - (horizontalSpacing * 2 + paddingBound * 2)) / mTtagCountOfRow;

		int verticalSpacing = dip2px(mContext, mContext.getResources().getDimension(R.dimen.tag_vertical_spacing));//垂直间隙
		int listviewPaddingTop =dip2px(mContext, mContext.getResources().getDimension(R.dimen.tag_listview_padding_top));//taglistview距离顶部间隙
		int listviewBottomTop = dip2px(mContext, mContext.getResources().getDimension(R.dimen.tag_common_margin));//taglistview距离底部间隙
	...
	public void setTags(List<? extends Tag> lists, boolean b) {
		...
		int itemHeight = dip2px(mContext , mContext.getResources().getDimension(R.dimen.tag_height));//每一个item高度
		int verticalSpacing = dip2px(mContext, mContext.getResources().getDimension(R.dimen.tag_vertical_spacing));//垂直间隙
		int listviewPaddingTop =dip2px(mContext, mContext.getResources().getDimension(R.dimen.tag_listview_padding_top));//taglistview距离顶部间隙
		int listviewBottomTop = dip2px(mContext, mContext.getResources().getDimension(R.dimen.tag_common_margin));//taglistview距离底部间隙
